import { McpUiHostContextSchema, McpUiHostStylesSchema } from '@modelcontextprotocol/ext-apps/app-bridge';
...
export function injectPreviewTheme(context) {
  ...
  let fonts = document.querySelector('style[data-preview-fonts]');
  if (!fonts) {
    fonts = document.createElement('style'); fonts.dataset.previewFonts = '';
    document.head.append(fonts);
  }
  fonts.textContent = parsed.styles?.css?.fonts || '';
  root.style.setProperty('--interaction-cursor', parsed['openai/interactionCursor'] || '');
}

export function observePreviewTheme(onTheme, onMissing) {
  let previous;
  const refresh = () => {
    ...
    const context = { theme: root.dataset.theme, styles: { variables,
      css: { fonts: document.querySelector('style[data-preview-fonts]')?.textContent || '' } },
